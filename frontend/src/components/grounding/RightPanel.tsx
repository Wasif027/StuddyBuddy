...
export function RightPanel() {
  ...
  // On phones the fixed-width side column would eat almost the whole screen,
  // so it defaults closed regardless of the persisted desktop preference, and
  // opening it shows a full-screen overlay instead of squeezing the chat.
  const [mobileOpen, setMobileOpen] = useState(false);
  const open = isMobile ? mobileOpen : rightOpen;
  const toggle = isMobile ? () => setMobileOpen((v) => !v) : toggleRight;
  ...
  // Phones get a full-screen overlay (no ResizeHandle, no fixed pixel width
  // squeezing the chat) instead of the desktop side column.
  if (isMobile) {
    return (
      <div className="fixed inset-0 z-50 flex flex-col bg-surface-raised">
        {body}
      </div>
    );
  }
